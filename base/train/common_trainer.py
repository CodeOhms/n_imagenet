...
class CommonTrainer(MiniBatchTrainer):
    ...
    def init_scheduler(self, **kwargs):
        ...
        """
        self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, 'max', verbose=True, patience=3)
    ...
    def run_test(self):
        ...
        for batch_idx, data_dict in enumerate(self.data_container.dataloader['test']):
            ...
            write_dict = {'top 1 test accuracy': test_acc_1, 'top 5 test accuracy': test_acc_5}
            print_dict = {'Iter': batch_idx, 'top 1 test acc': test_acc_1, 'top 5 test acc': test_acc_5, 'load time': load_time}
